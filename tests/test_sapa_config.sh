...
for key in base remote pr tracker plan build writing_style watch gate; do
  present=no
...
when=no
grep -qE "^#       when: pre-push .*always.*default" "$fresh/.sapa.yaml" && when=yes
check "init documents gate step when:" "yes" "$when"
